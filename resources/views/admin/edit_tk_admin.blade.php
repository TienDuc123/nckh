...
<body>
	...
	<!-- Large modal -->
	<div class="container">
		<form id="uploadForm_ud" enctype="multipart/form-data" action="{{route('updatetaikhoanmoi_admin')}}" method="POST">
			@csrf	
		    <div class="row">
		      ...
		      <div class="col-6">
		      	...
		      	<input type="text" id="name_nv" name="name_nv" class="form-control" required value="{{isset($user->ten_admin)?$user->ten_admin:''}}">
		      </div>
		      <div class="col-6">
		      	<label for="ma_nv" class="btn-sm-2">Mã Khách Hàng</label>
		      	<input type="text" id="ma_nv" name="ma_nv" class="form-control" required value="{{isset($user->ma_admin)?$user->ma_admin:''	}}">
		      </div>
		      <div class="col-6">
